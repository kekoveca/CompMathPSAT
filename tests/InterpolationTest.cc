...
Eigen::Array<xType, N, 1> chebyshevZeros(xType a, xType b){
    Eigen::Array<xType, N, 1> zeros;
...
TEST(InterpolantTests, ChebyshevTest){
    std::ofstream fileOutChebN345;

    fileOutChebN345.open(dataPath + "/errChebN345.txt");
    ...
    for(int i = 0; i < power; i++) {
        pointsMatrix3.col(i) = chebyshevZeros<double, 3>(0., std::pow(2., 1 - i));
        valuesMatrix3.col(i) = pointsMatrix3.col(i).exp();
    }
    ...
    for(int i = 0; i < power; i++) {
        pointsMatrix4.col(i) = chebyshevZeros<double, 4>(0., std::pow(2., 1 - i));
        valuesMatrix4.col(i) = pointsMatrix4.col(i).exp();
    }
    ...
    for(int i = 0; i < power; i++) {
        pointsMatrix5.col(i) = chebyshevZeros<double, 5>(0., std::pow(2., 1 - i));
        valuesMatrix5.col(i) = pointsMatrix5.col(i).exp();
    }
